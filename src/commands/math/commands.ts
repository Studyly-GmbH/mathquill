...
LatexCmds.overrightarrow = () =>
    new Style(
        ...
        {
            afterChild: () =>
                h('span', { class: 'mq-arrow-right-content' }, [
                    h.text(ArrowText)
                ])
        }
    );
LatexCmds.overleftarrow = () =>
    new Style(
        ...
        {
            beforeChild: () =>
                h('span', { class: 'mq-arrow-left-content' }, [
                    h.text(ArrowText)
                ])
        }
    );
    ...
    deleteTowards(dir: Direction, cursor: Cursor) {
        if (cursor.options.autoSubscriptNumerals && this.sub) {
            ...
            if (cmd instanceof MQSymbol) cmd.remove();
                ...
                cmd.deleteTowards(
                    dir,
                    cursor.insAtDirEnd(-dir as Direction, this.sub)
                );
    ...
    addBlock(block: MathBlock) {
        ...
        for (var i = 0; i < 2; i += 1)
            (function (
                ...
                cmdSubSub.deleteOutOf = function (
                    dir: Direction,
                    cursor: Cursor
                ) {
                    cursor.insDirOf(
                        this[dir] ? (-dir as Direction) : dir,
                        this.parent
                    );
                    if (!this.isEmpty()) {
                    ...
                    cmdOppositeSupsub[`${updown}OutOf`] =
...
class Bracket extends DelimsNode {
    ...
    matchBrack(
        ...
        return (
            ...
            (!opts.restrictMismatchedBrackets ||
                OPP_BRACKS[
                    this.sides[this.side as Direction]
                        .ch as keyof typeof BRACKET_NAMES
    ...
    closeOpposing(brack: Bracket) {
        brack.side = 0;
        brack.sides[this.side as Direction] =
            this.sides[this.side as Direction]; // copy over my info (may be
