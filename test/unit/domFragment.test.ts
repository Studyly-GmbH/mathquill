suite('DOMFragment', function () {
    ...
    suite('.children()', () => {
        ...
        test('returns a fragment representing all children', () => {
            ...
            assert.ok(
                nodeArraysEqual(
                    domFrag(parent).children().toNodeArray(),
                    children
                )
            );
        });
    ...
    suite('.join()', () => {
        ...
        test('Joining fragments that are siblings but not directional siblings throws', () => {
            ...
            h('span', {}, children);
            assert.throws(() =>
                domFrag(children[2]).join(domFrag(children[0]))
            );
            assert.throws(() =>
    ...
    suite('.toElementArray()', () => {
        ...
        test('returns an array of each element in a multi-node fragment', () => {
            ...
            assert.ok(
                nodeArraysEqual(
                    domFrag(parent).children().toElementArray(),
                    expected
                )
            );
        });
    ...
    for (const { name, apply } of [
        ...
        suite(name, () => {
            test('is a noop when called on an empty fragment', () => {
                ...
                assert.ok(
                    apply(domFrag(), domFrag(parent).children()).isValid()
                );
                assert.ok(
                    nodeArraysEqual(
                        domFrag(parent).children().toNodeArray(),
            ...
            test('detaches this fragment when called with an empty fragment', () => {
                ...
                assert.ok(
                    apply(domFrag(parent).children(), domFrag()).isValid()
                );
                assert.ok(
                    domFrag(parent).children().isEmpty(),
    ...
    for (const { name, apply } of [
        ...
        suite(name, () => {
            test('is a noop when this is an empty fragment', () => {
                ...
                assert.ok(
                    apply(domFrag(), domFrag(parent).oneElement()).isValid()
                );
                assert.ok(
                    nodeArraysEqual(
                        domFrag(parent).children().toNodeArray(),
    ...
    suite('.parent()', () => {
        ...
        test('returns the parent of a one element fragment', () => {
            ...
            const parent = h('span', {}, children);
            assert.equal(
                domFrag(parent).children().parent().oneElement(),
                parent
            );
        });
    ...
    suite('.wrapAll()', () => {
        test("removes all of target's children when called with an empty fragment", () => {
            ...
            assert.ok(!domFrag(target).children().isEmpty());
            assert.ok(
                domFrag().wrapAll(domFrag(target).oneElement()).isValid()
            );
            assert.ok(domFrag(target).children().isEmpty());
        });
        test("replaces target's children with this fragment and places target into the DOM at the previous location of this fragment", () => {
            ...
            assert.ok(
                nodeArraysEqual(
                    domFrag(sourceParent).children().toNodeArray(),
                    [sourceChildren[0], target]
                )
            );
            assert.ok(
    ...
    suite('.replaceWith()', () => {
        test('detaches source when this is an empty fragment', () => {
            ...
            assert.ok(
                domFrag()
                    .replaceWith(domFrag(sourceParent).children())
                    .isValid()
            );
            assert.ok(domFrag(sourceParent).children().isEmpty());
        });
        ...
        test('replaces this fragment with the fragment it is called with', () => {
            ...
            assert.ok(!domFrag(sourceParent).children().isEmpty());
            assert.ok(
                frag.replaceWith(domFrag(sourceParent).children()).isValid()
            );
            assert.ok(domFrag(sourceParent).children().isEmpty());
            assert.ok(
                nodeArraysEqual(
    ...
    suite('.toggleClass()', () => {
        ...
        test('is a noop for a fragment with no elements', () => {
            ...
            assert.ok(
                domFrag(h.text('text')).toggleClass('cls', true).isValid()
            );
            assert.ok(
                domFrag(h.text('text')).toggleClass('cls', false).isValid()
            );
        });
