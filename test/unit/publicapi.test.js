suite('Public API', function () {
    ...
    suite('basic API methods', function () {
        ...
        test('.mathspeak()', function () {
            ...
            mq.latex('')
                .typedText('\\langle')
                .keystroke('Spacebar')
                .typedText('u,v'); // .latex() doesn't work yet for angle brackets :(
            assertMathSpeakEqual(
                mq.mathspeak(),
    ...
    suite('*OutOf handlers', function () {
        ...
        testHandlers(
            ...
            function (options) {
                return MQ.StaticMath(
                    $('<span>\\MathQuillMathField{}</span>').appendTo(
                        '#mock'
                    )[0]
    ...
    suite('.cmd(...)', function () {
        ...
        test('basic', function () {
            ...
            mq.keystroke('Right Shift-Left Shift-Left Shift-Left').cmd(
                '\\sqrt'
            );
    ...
    suite('leftRightIntoCmdGoes: "up"/"down"', function () {
        test('"up" or "down" required', function () {
            assert.throws(function () {
                MQ.MathField($('<span></span>')[0], {
                    leftRightIntoCmdGoes: 1
                });
            });
        });
        suite('default', function () {
    ...
    suite('substituteKeyboardEvents (interface versions 1 and 2)', function () {
        for (const v of [1, 2]) {
            ...
            test(
                'can intercept key events, interface version ' + v,
                function () {
                    var mq = MQ_old.MathField(
                        $('<span>').appendTo('#mock')[0],
                        {
                                ...
                                return MQ_old.saneKeyboardEvents(
                                    ...
                                    $.extend({}, handlers, {
                                        keystroke: function (_key, evt) {
                                            key = _key;
                                            return handlers.keystroke.apply(
                                                handlers,
                                                arguments
                                            );
                                        }
    ...
    suite('StaticMath', function () {
        test('does not render cursor', function () {
            ...
            var mq = MQ.StaticMath(
                domFrag(span)
                    .appendTo(document.querySelector('#mock'))
                    .oneElement(),
                {}
            );
            assert.equal(
